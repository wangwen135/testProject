 ...
 * @since 1.0
 */
public class GroovyEntityEditor extends EntityAbstractDialog<GroovyEntity> {

	private static Logger logger = LoggerFactory.getLogger(GroovyEntityEditor.class);
	...
	private static GroovyEntityEditor editor;

	public static synchronized GroovyEntityEditor getInstance(Window windows) {
		if (editor == null) {
			editor = new GroovyEntityEditor(windows);
		}
		return editor;
	}
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GroovyEntityEditor frame = new GroovyEntityEditor(null);
	 ...
	 * Create the frame.
	 */
	public GroovyEntityEditor(Window windows) {
		super(windows);
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					...
					JOptionPane.showMessageDialog(GroovyEntityEditor.this,
							"创建脚本引擎失败！\r\n" + e.getMessage());
				}
			}
		});
	...
	public void modelToView() {
		textPane_script.setText(entity.getScript());

	}

	@Override
	public void viewToModel() {
		entity.setScript(textPane_script.getText());
	}

	@Override
	public void showEdit(GroovyEntity entity) {
		this.entity = entity;
		modelToView();
		clear();
