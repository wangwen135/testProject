package com.swing.edi.reportModel.utils;
...
public class ReportModelUtils {
	...
	public static EdiTableModel getTableModelFromXml(Element area) {

		int column = Integer.valueOf(area.attributeValue("columnCount"));
		int row = Integer.valueOf(area.attributeValue("rowCount"));

		EdiTableModel tableModel = new EdiTableModel(row, column);
		Vector<Vector<ICell>> data = tableModel.getDataVector();

		for (Iterator<Element> iterator = area.elementIterator(); iterator
				.hasNext();) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static void setAreaValue(EdiTableModel tm, Map<String, String> row) {
